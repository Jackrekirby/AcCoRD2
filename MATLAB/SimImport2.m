classdef SimImport2 < handle
    properties
        s
    end
    
    methods (Access = public)
        function obj = SimImport2(directory, simulationName, seeds, ...
            realisations)
            arguments
                directory
                simulationName
            ...
            simulationDir = ...
                SimImport2.isSimulationDirectoryValid(directory, simulationName);
    ...
    methods (Access = private)
        function findSeeds(obj, simulationDir, seeds)
            if(isempty(seeds))
                seeds = SimImport2.GetSuffixesFromFolderNames(simulationDir, 's');
                if(isempty(seeds))
        ...
        function findRealisations(obj, simulationDir, realisations)
            ...
            for iSeed = 1:length(obj.s)
                if(isempty(realisations))
                    ...
                    realisations = SimImport2.GetSuffixesFromFolderNames(seedDir, 'r');
                    if(isempty(realisations))
                ...
                if(endsWith(filePath,extension))
                    ...
                    if(actorType == 'p')
                        ...
                        if(timeOrmole == 't')
                            ...
                            SimImport2.readTimeBinary(files(iFile).folder + "\" + files(iFile).name);
                        elseif(timeOrmole == 'm')
                            ...
                            if(fileType == 'p')
                                ...
                                if(isfile(files(iFile).folder + "\" + files(iFile).name(1:end-5) + "c.bin"))
                                    ...
                                    obj.s(iSeed).r(iRealisation).p(actorID + 1).m(moleID + 1) = ...
                                        SimImport2.readPositionBinary(files(iFile).folder + "\" + files(iFile).name(1:end-5));
                                end
                            elseif(fileType == 'c')
                                ...
                                if(~isfile(files(iFile).folder + "\" + files(iFile).name(1:end-5) + "p.bin"))
                                    ...
                                    obj.s(iSeed).r(iRealisation).p(actorID + 1).m(moleID + 1).c = ...
                                        SimImport2.readCountBinary(files(iFile).folder + "\" + files(iFile).name);
                                end
                            end
                        end
                    elseif(actorType == 'a')
