...
for i = 1:length(simulation.passiveActor.moleType)
    if(length(simulation.passiveActor.moleType(i)) > n_molecule_types)
        n_molecule_types = length(simulation.passiveActor.moleType(i));
    end
end
...
while(min(simulation.passiveActorTimes) ~= Inf)
    ...
    if(time > simulation.time)
        ...
        for i = 1:length(simulation.actorsToRender)
            actor_id = simulation.actorsToRender(i);
            for j = 1:length(simulation.passiveActor(actor_id).moleType)
                if(sum(simulation.passiveActor(actor_id).moleType(j).count) ~= 0)
...
for i = 1:length(simulation.passiveActor)
    for j = 1:length(simulation.passiveActor(i).moleType)
        if(~isempty(simulation.passiveActor(i).moleType(j).count) && ...
                sum(simulation.passiveActor(i).moleType(j).count) ~= 0)
