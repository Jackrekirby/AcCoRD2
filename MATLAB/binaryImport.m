...
simulation.hFigure = figure('visible','off');
view(3);
xlabel('x');
ylabel('y');
zlabel('z');
...
for i = 1:size(simulation.passiveActor, 2)
    ...
    if(isfield(simulation.passiveActor(i), 'times'))
        ...
        simulation.passiveActor(i).hPlot = plot3(0, 0, 0, 'Marker', 'o');
        simulation.passiveActor(i).hPlot.XData = [];
        simulation.passiveActor(i).hPlot.YData = [];
        simulation.passiveActor(i).hPlot.ZData = [];
    else
        ...
        for i = 1:size(simulation.actorsToRender, 2)
            ...
            for j = 1:size(simulation.passiveActor(actor_id).moleType, 2)
                if(simulation.passiveActor(actor_id).moleType(j).count ~= 0)
                    ...
                    simulation.passiveActor(actor_id).hPlot.XData = positions(1, :);
                    simulation.passiveActor(actor_id).hPlot.YData = positions(2, :);
                    simulation.passiveActor(actor_id).hPlot.ZData = positions(3, :);
                end
            end
        end
